...
/* ================================================================
   ...
   ================================================================ */
// Al estar aquí afuera, TODAS las funciones pueden usar la URL
const BACKEND_URL =
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  btnTabHistorial.addEventListener("click", () => {
    ...
    cargarHistorial(); // Refrescar datos al abrir la pestaña
  });
  ...
  // ==============================================================
  let historialGlobal = [];
  let registrosFiltrados = []; // Para guardar los datos a imprimir
  ...
  const renderizarHistorial = () => {
    ...
    registrosFiltrados = historialGlobal.filter((item) => {
      let matchFecha = true;
      if (fechaExacta) {
        matchFecha = item.fecha === fechaExacta;
      } else if (mes !== "ALL") {
        matchFecha = item.fecha && item.fecha.startsWith(mes);
      }
  ...
  // ==============================================================
  // MOTOR DE REPORTES (IMPRESIÓN Y PDF - PORTRAIT)
  // ==============================================================
  const generarHTMLReporte = (isForPDF = false) => {
    ...
    // Ordenamiento Cronológico Robusto (Ascendente)
    ...
    for (let i = 0; i < totalPaginas; i++) {
      ...
      chunk.forEach(r => {
        ...
        // VALORES EN CRUDO (Sin toFixed, sin recortes)
        const cantHtml = tieneDosificacion ? r.dosificacion : ''; // Crudo
        const cloroHtml = r.cloro; // Crudo
        const monitoreador = r.usuario || ''; // Crudo (ej: operario@planta.com)
        
        // Mantener la fecha completa (DD/MM/YYYY) sin recortar el año
  ...
  filterPunto.addEventListener("change", renderizarHistorial);
  ...
  // Primera carga al abrir el sistema
  cargarHistorial();
});
